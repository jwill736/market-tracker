...
# First run: creates a Python environment, installs the app and a .env file. Then opens the dashboard.
set -euo pipefail
cd "$(dirname "$0")"
PY=${PYTHON:-python3}
...
if [ ! -f .env ]; then
  cp .env.example .env
  echo "Created .env. Put your email in SEC_USER_AGENT there (the SEC asks for a contact)."
fi
